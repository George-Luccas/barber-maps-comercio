...
export default function FinancialManager() {
  ...
  const [viewMode, setViewMode] = useState<"TRANSACTIONS" | "CLIENTS">("TRANSACTIONS");
      ...
      <div className="max-w-4xl mx-auto w-full flex flex-col md:flex-row items-start md:items-center justify-between mb-4 gap-4">
        ...
        <div className="flex bg-muted p-1 rounded-xl">
            <button 
                ...
                className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === "TRANSACTIONS" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
            >
                Caixa
            </button>
            <button 
                onClick={() => setViewMode("CLIENTS")}
                className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === "CLIENTS" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
            >
                Clientes
            </button>
        </div>
