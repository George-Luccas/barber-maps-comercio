...
export const authConfig = {
  ...
  callbacks: {
    authorized({ auth, request: { nextUrl } }) {
      ...
      const isPublicPath = ["/login", "/register", "/landing", "/test-db", "/api", "/forgot-password", "/reset-password", "/manifest.json", "/robots.txt", "/sitemap.xml"].some(p => nextUrl.pathname.startsWith(p));
