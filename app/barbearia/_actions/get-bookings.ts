...
export async function getBookings(barbershopId: string, dateStr?: string) {
  ...
  try {
    ...
    console.error("Erro com include user, tentando sem:", error);
    try {
        // Fallback: Busca sem o include do usuário se falhar (ex: erro de relação entre bancos)
        const bookingsNoUser = await db.booking.findMany({
            where: {
                barbershopId: barbershopId,
                date: {
                    gte: startOfDay,
        ...
        return mapBookings(bookingsNoUser);
    } catch (retryError) {
        console.error("Erro fatal ao buscar agendamentos:", retryError);
        return [];
    }
  }
}

function mapBookings(bookings: any[]) {
